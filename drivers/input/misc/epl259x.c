...
#define PS_LOW_THRESHOLD		    160//500 
#define PS_HIGH_THRESHOLD		 510//800 

#define LUX_PER_COUNT			196
...
#define QCOM_PWR    1
#define QCOM_FACTORY    1
#define ALS_STOP_PATCH  1
...
#define EPL259x_VDD_MIN_UV  2000000
#define EPL259x_VDD_MAX_UV  3300000
#define EPL259x_VIO_MIN_UV  1750000
#define EPL259x_VIO_MAX_UV  1950000
#endif
...
#if PACKET_MODE
static  int dyn_k_ps_pocket = 10000;
static  int dyn_k_ps_max_ct = 10000;
...
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_1024, EPL_ALS_INTT_64};
static int als_dynamic_intt_value[] = {1024, 64};
...
#define LOG_FUN(f)               	 pr_debug(KERN_INFO LOG_TAG"%s\n", __FUNCTION__)
#define LOG_INFO(fmt, args...)    	 pr_debug(KERN_INFO LOG_TAG fmt, ##args)
...
#if QCOM_PWR
static int elan_plsensor_power_set(struct epl_sensor_priv *epld, bool on);
static int elan_power_init(struct epl_sensor_priv *epld, bool on);
#endif
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
	...
	epl_sensor.wait = EPL_WAIT_75_MS;//EPL_WAIT_20_MS;
	...
	epl_sensor.ps.integration_time = EPL_PS_INTT_1040;//EPL_PS_INTT_520;
	epl_sensor.ps.gain = EPL_GAIN_LOW;
#if PS_DYN_K
    dynk_min_ps_raw_data = 0xffff; //min raw data
    dynk_max_ir_data = 50000;      //max ch0
    dynk_low_offset = 720;         //low thd offset 500 
    dynk_high_offset = 1850;        //high thd offset 800
	...
	epl_sensor.ps.ir_drive = EPL_IR_DRIVE_50;//EPL_IR_DRIVE_100;
    ...
    epl_sensor.ges.integration_time = EPL_PS_INTT_144;
...
void epl_sensor_dynk_thd_polling_work(struct work_struct *work)
{
    ...
    {
#if ALS_STOP_PATCH
        u8 buf = 0, als_mode = 0, reg0x11;
        mutex_lock(&sensor_mutex);
        epl_sensor_I2C_Read(obj->client, 0x00, 1);
        buf = gRawData.raw_bytes[0];
        als_mode = (buf & 0x03);
        epl_sensor_I2C_Read(obj->client, 0x11, 1);
        buf = gRawData.raw_bytes[0];
        reg0x11 = (buf & 0x03);
        mutex_unlock(&sensor_mutex);

        LOG_INFO("[%s]: REG0x00=0x%x, als_mode=0x%x, epl_sensor.mode=0x%x, reg0x11=0x%x\r\n", __func__, buf, als_mode, epl_sensor.mode, reg0x11);
        if( (als_mode != epl_sensor.mode) || ( reg0x11 != (EPL_POWER_ON | EPL_RESETN_RUN) ))
        {
            mutex_lock(&sensor_mutex);
        ...
        if( (dynk_min_ps_raw_data > epl_sensor.ps.data.data)
            && (epl_sensor.ps.saturation == 0)
            && (epl_sensor.ps.data.ir_data < dynk_max_ir_data)
            && (epl_sensor.ps.data.data < dyn_k_ps_max_ct) )
...
#if PACKET_MODE
        else if ((epl_sensor.ps.data.data > dyn_k_ps_max_ct) && (packet_flag == true))
        {
            packet_flag = false;
            mutex_lock(&sensor_mutex);
		    set_psensor_intr_threshold((u16)dynk_thd_low, (u16)dynk_thd_high);
            mutex_unlock(&sensor_mutex);
            LOG_INFO("[%s]: recover last thd L/H=%ld/%ld, epl_sensor.ps.data.data=%d \r\n", __func__, (long)dynk_thd_low, (long)dynk_thd_high, epl_sensor.ps.data.data);
        }
#endif
...
static void epl_sensor_polling_work(struct work_struct *work)
{
    ...
    if(enable_als &&  epl_sensor.als.polling_mode == 1)
    {
	    int report_lux = 0;
#if ALS_STOP_PATCH
        u8 buf = 0, als_mode = 0, reg0x11 = 0;
        mutex_lock(&sensor_mutex);
        epl_sensor_I2C_Read(epld->client, 0x00, 1);
        buf = gRawData.raw_bytes[0];
        als_mode = (buf & 0x03);
		epl_sensor_I2C_Read(epld->client, 0x11, 1);
        buf = gRawData.raw_bytes[0];
        reg0x11 = (buf & 0x03);
        mutex_unlock(&sensor_mutex);
        LOG_INFO("[%s]: REG0x00=0x%x, als_mode=0x%x, epl_sensor.mode=0x%x, reg0x11=0x%x\r\n", __func__, buf, als_mode, epl_sensor.mode, reg0x11);
...
#if ALS_DYN_INTT
    if(epld->enable_lflag == 1 && epl_sensor.als.polling_mode == 1)
    {
        do
        {
            //mutex_lock(&sensor_mutex);
            epl_sensor_read_als(epld->client);
            //mutex_unlock(&sensor_mutex);
            epl_sensor_get_als_value(epld, epl_sensor.als.data.channels[1]);
...
//            wake_lock(&ps_lock);
...
#if PACKET_MODE
            packet_flag = true;
            set_psensor_intr_threshold(dyn_k_ps_pocket, dyn_k_ps_pocket+1);
#else
            set_psensor_intr_threshold(65534, 65535);   ////dont use first ps status
#endif
            dynk_min_ps_raw_data = 0xffff;
        ...
        {
#if PS_DYN_K
			input_report_abs(epld->ps_input_dev, ABS_DISTANCE, 0xff);
    		input_sync(epld->ps_input_dev);
...
static int elan_plsensor_power_set(struct epl_sensor_priv *epld, bool on)
{
	...
	//	msleep(50); // L6 L17 has been set always on
		return rc;
	} else {
...
static int elan_power_init(struct epl_sensor_priv *epld, bool on)
{
	...
	if (regulator_count_voltages(epld->vdd) > 0) {
		rc = regulator_set_voltage(epld->vdd, EPL259x_VDD_MIN_UV,
					   EPL259x_VDD_MAX_UV);
		if (rc) {
			dev_err(&epld->client->dev,
	...
	if (regulator_count_voltages(epld->vio) > 0) {
		rc = regulator_set_voltage(epld->vio, EPL259x_VIO_MIN_UV,
					   EPL259x_VIO_MAX_UV);
		if (rc) {
			dev_err(&epld->client->dev,
...
reg_vdd_set_vtg:
	if (regulator_count_voltages(epld->vdd) > 0)
		regulator_set_voltage(epld->vdd, 0, EPL259x_VDD_MAX_UV);
reg_vdd_put:
...
pwr_deinit:
	...
	if (regulator_count_voltages(epld->vio) > 0)
		regulator_set_voltage(epld->vio, 0, EPL259x_VIO_MAX_UV);
